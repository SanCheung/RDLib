...
CStringW* CTextUI::GetLinkContent(int iIndex)
{
...
void CTextUI::PaintText(HDC hDC)
{
	...
	if( IsEnabled() ) {
		if( m_bShowHtml )
			...
			m_iFont, m_rcLinks, m_sLinks, m_nLinks, m_uTextStyle);
		else
...
void CTextUI::SetAttribute( LPCTSTR pstrName, LPCTSTR pstrValue )
{
	...
	else if( EQUSTR(pstrName, _T("hcenter")) ) 
	{
		if( EQUSTR(pstrValue, _T("true")) ) {
