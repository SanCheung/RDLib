...
{
	m_fontLib.SetDefaultFont( pStrFontName, nSize, bBold, bUnderline, bItalic );
...
{
	return m_fontLib.GetCustomFontCount();
...
{
	return m_fontLib.AddFont( pStrFontName, nSize, bBold, bUnderline, bItalic );
 //   LOGFONT lf = { 0 };
 //   ::GetObject(::GetStockObject(DEFAULT_GUI_FONT), sizeof(LOGFONT), &lf);
 //   _tcscpy(lf.lfFaceName, pStrFontName);
 //   lf.lfCharSet = DEFAULT_CHARSET;
...
{
	HFONT	hFt = m_fontLib.CloneFont( hFont );
	if( hFt == NULL )
	{
		if( m_pParentResourcePM ) 
			hFt = m_pParentResourcePM->CloneFont(hFont);
	}
...
HFONT CPaintManagerUI::GetFont(int index)
{
	...
	return m_fontLib.GetFont( index );
...
{
	HFONT	ft = m_fontLib.GetFont( pStrFontName, nSize, bBold, bUnderline, bItalic );
	if( ft == NULL )
	{
		if( m_pParentResourcePM ) 
...
bool CPaintManagerUI::FindFont(HFONT hFont)
{
 ...
 //   return false;
	...
	bool bFound = m_fontLib.FindFont( hFont );
	if( bFound )
		return true;
...
bool CPaintManagerUI::FindFont(LPCTSTR pStrFontName, int nSize, bool bBold, bool bUnderline, bool bItalic)
{
 ...
 //   return false;
	...
	bool bFound = m_fontLib.FindFont(  pStrFontName, nSize, bBold, bUnderline, bItalic );
	if( bFound )
		return true;
...
int CPaintManagerUI::GetFontIndex(HFONT hFont)
{
   ...
   // return -1;
	...
	return m_fontLib.GetFontIndex( hFont );
}
...
int CPaintManagerUI::GetFontIndex(LPCTSTR pStrFontName, int nSize, bool bBold, bool bUnderline, bool bItalic)
{
   ...
   // }
   // return -1;
	return m_fontLib.GetFontIndex( pStrFontName, nSize, bBold, bUnderline, bItalic );
}

int CPaintManagerUI::GetFontIndexEx( LPCTSTR pStrFontName, int nSize, bool bBold, bool bUnderline, bool bItalic )
{
	return m_fontLib.GetFontIndexEx(pStrFontName,nSize, bBold, bUnderline,bItalic);
	//TFontInfo* pFontInfo = NULL;
	//for( int it = 0; it < (int)m_aCustomFonts.size(); it++ )
	//{
	//	pFontInfo = static_cast<TFontInfo*>(m_aCustomFonts[it]);
...
{
	return m_fontLib.RemoveFont( hFont );
 //   TFontInfo* pFontInfo = NULL;
 ////   for( int it = 0; it < (int)m_aCustomFonts.size(); it++ ) 
 ////	  {
 ////       pFontInfo = static_cast<TFontInfo*>(m_aCustomFonts[it]);
...
{
	return m_fontLib.RemoveFontAt( index );
...
{
	return m_fontLib.GetFontInfo( index );
...
TFontInfo* CPaintManagerUI::GetFontInfo(HFONT hFont)
{
	...
	return m_fontLib.GetFontInfo( hFont );
