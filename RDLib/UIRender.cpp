...
#include "../other/stimage_static/stb_image.h"
...
TImageInfo* CRenderEngine::LoadImage(STRINGorID bitmap, LPCTSTR type, DWORD mask)
{
    ...
    if( type == NULL ) {
        //CStringW sFile = CPaintManagerUI::GetResourcePath();
		CStringW sFile = CAppData::GetResourcePath();
        if( CAppData::GetResourceZip().IsEmpty() ) {
            ...
            HZIP hz = NULL;
            if( CAppData::IsCachedResourceZip() ) hz = (HZIP)CAppData::GetResourceZipHandle();
            ...
            if( res != 0x00000000 && res != 0x00000600) {
                delete[] pData;
                if( !CAppData::IsCachedResourceZip() ) CloseZip(hz);
                return NULL;
            }
            if( !CAppData::IsCachedResourceZip() ) CloseZip(hz);
        }
        ...
        HRSRC hResource = ::FindResource(CAppData::GetResourceDll(), bitmap.m_lpstr, type);
        if( hResource == NULL ) return NULL;
        HGLOBAL hGlobal = ::LoadResource(CAppData::GetResourceDll(), hResource);
        if( hGlobal == NULL ) {
            FreeResource(hResource);
            return NULL;
        }

        dwSize = ::SizeofResource(CAppData::GetResourceDll(), hResource);
        if( dwSize == 0 ) return NULL;
        pData = new BYTE[ dwSize ];
	...
	TImageInfo* data = LoadImageFromBuffer(pData, dwSize, mask);
	delete []pData;
	return data;
}
                                          ...
                                          LPCTSTR pStrImage, LPCTSTR pStrModify)
{
    ...
    CStringW sImageName = pStrImage;
    CStringW sImageResType;
    RECT rcItem = rc;
    RECT rcBmpPart = {0};
    RECT rcCorner = {0};
    DWORD dwMask = 0;
        ...
        while( *pStrImage != _T('\0') ) {
            ...
            if( !sValue.IsEmpty() ) {
                if( sItem == _T("file") || sItem == _T("res") ) {
					if( image_count > 0 )
						rdDrawImage(hDC, pManager, rc, rcPaint, sImageName, sImageResType,
                ...
                else if( sItem == _T("mask") )
				{
                    ...
                    bHole = EQUSTR_TRUE(sValue);
                else if( sItem == _T("xtiled") )
                    bTiledX = EQUSTR_TRUE(sValue);
                else if( sItem == _T("ytiled") )
                    bTiledY = EQUSTR_TRUE(sValue);
            }
...
RECT AdjustForVerticalAlignment(HDC hdc, LPCTSTR text, RECT bounds, DWORD flags, LPDRAWTEXTPARAMS dtparams)
{
	...
	RECT lpRect = {bounds.left, bounds.top, bounds.left + bounds.right, bounds.top + bounds.bottom};
	flags |= DT_CALCRECT;
	int num = DrawTextEx(hdc, (LPTSTR)text, -1, &lpRect, (int) flags, dtparams);
	if (num > bounds.bottom)
	{
		return bounds;
	}
	RECT rectangle = bounds;
	if ((flags & DT_VCENTER) != DT_TOP)
	{
		rectangle.top = (rectangle.top + (rectangle.bottom / 2)) - (num / 2);
		return rectangle;
	}
	rectangle.top = rectangle.top + rectangle.bottom - num;
...
{		
	...
	int		nType = ::GetObjectType(hDC);
    //ASSERT( nType ==OBJ_DC || nType==OBJ_MEMDC);
	...
	{
		RECT bounds = {rc.left, rc.top, rc.right - rc.left, rc.bottom - rc.top}; //rectangle as gdi+
		bounds = AdjustForVerticalAlignment(hDC, pstrText, bounds, uStyle, NULL);
		if (bounds.right - bounds.left == MaxSize.cx)
		{
			bounds.right -= bounds.left;
		}
		if (bounds.bottom - bounds.top == MaxSize.cy)
		{
			bounds.bottom -= bounds.top;
		}
		//计算rect大小的时候 DT_WORDBREAK
		RECT rc1 = {bounds.left, bounds.top, bounds.left + bounds.right, bounds.top + bounds.bottom};
		DrawTextEx(hDC, (LPTSTR)pstrText, -1, &rc1, uStyle, NULL);
		CopyRect(&rc, &rc1);
	}
...
void CRenderEngine::DrawHtmlText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, LPCTSTR pstrText, DWORD dwTextColor, int iFont, RECT* prcLinks, CStringW* sLinks, int& nLinkRects, UINT uStyle)
{
    ...
    if( (uStyle & DT_SINGLELINE) != 0 && (uStyle & DT_VCENTER) != 0 && (uStyle & DT_CALCRECT) == 0 ) {
        ...
        int nLinks = 0;
        DrawHtmlText(hDC, pManager, rcText, pstrText, dwTextColor, iFont, NULL, NULL, nLinks, uStyle | DT_CALCRECT);
	...
	if( !aPIndentArray.empty() ) cyLine += (int)aPIndentArray.back();
    int cyMinHeight = 0;
    int cxMaxWidth = 0;
    POINT ptLinkStart = { 0 };
    bool bLineEnd = false;
	...
	vectorv aLineFontArray;
	vectorv aLineColorArray;
	vectorv aLinePIndentArray;
    ...
    while( *pstrText != _T('\0') ) {
        if( pt.x >= rc.right || *pstrText == _T('\n') || bLineEnd ) {
            ...
            if( !bLineDraw ) {
                if( bInLink && iLinkIndex < nLinkRects ) {
                    ...
                    CStringW *pStr1 = (CStringW*)(sLinks + iLinkIndex - 1);
                    CStringW *pStr2 = (CStringW*)(sLinks + iLinkIndex);
                    *pStr2 = *pStr1;
                }
            ...
            case _T('a'):  // Link
                {
                    ...
                    if( iLinkIndex < nLinkRects && !bLineDraw ) {
                        CStringW *pStr = (CStringW*)(sLinks + iLinkIndex);
                        pStr->Empty();
                    ...
                    //        clrColor = pManager->GetDefaultLinkHoverFontColor();
                    //}
                    aColorArray.push_back((LPVOID)clrColor);
					...
					if( !aFontArray.empty() )
						pFontInfo = (TFontInfo*)aFontArray.back();
                    ...
                    if( pFontInfo->bUnderline == false )
					{
						...
						int new_cyLine = pTm->tmHeight + pTm->tmExternalLeading;
						if( !aPIndentArray.empty() ) new_cyLine += (int)aPIndentArray.back();

                        //cyLine = MAX(cyLine, pTm->tmHeight + pTm->tmExternalLeading + (int)aPIndentArray.GetAt(aPIndentArray.GetSize() - 1));
						cyLine = MAX(cyLine, new_cyLine);
                    }
                    ptLinkStart = pt;
                    bInLink = true;
                }
            ...
            case _T('i'):  // Italic or Image
                {    
                    ...
                    pstrText++;
					CStringW sImageString = pstrText;
                    int iWidth = 0;
                    int iHeight = 0;
                    while( *pstrText > _T('\0') && *pstrText <= _T(' ') ) pstrText = ::CharNext(pstrText);
                    ...
                    if( sName.IsEmpty() ) { // Italic
                        ...
                        TFontInfo* pFontInfo = pManager->GetDefaultFontInfo();
                        //if( aFontArray.GetSize() > 0 ) pFontInfo = (TFontInfo*)aFontArray.GetAt(aFontArray.GetSize() - 1);
						if( !aFontArray.empty() ) pFontInfo = (TFontInfo*)aFontArray.back();
						...
						int iImageListIndex = (int)
							_tcstol(pstrText, const_cast<LPTSTR*>(&pstrText), 10);
						if( iImageListIndex < 0 || iImageListIndex >= iImageListNum )
							iImageListIndex = 0;

						TCHAR *pstrTemp = sImageString;
						if( _tcsstr(pstrTemp, _T("file=\'")) != NULL || _tcsstr(pstrTemp, _T("res=\'")) != NULL )
						{
							CStringW sImageResType;
							CStringW sImageName;
            ...
            case _T('p'):  // Paragraph
                {
					...
					aPIndentArray.push_back((LPVOID)cyLineExtra);
            ...
            case _T('c'):
                {
					...
					if( !aColorArray.empty() ) clrColor = (int)aColorArray.back();
                    ::SetTextColor(hDC, RGB(GetBValue(clrColor), GetGValue(clrColor), GetRValue(clrColor)));
                }
                break;
            case _T('p'):
        ...
        if( bDraw && bLineEnd )
		{
            if( !bLineDraw )
			{
				...
				aFontArray.clear();
				copy( aLineFontArray.begin(), aLineFontArray.end(), back_inserter(aFontArray) );
				...
				aColorArray.clear();
				copy( aLineColorArray.begin(), aLineColorArray.end(), back_inserter(aColorArray) );
				...
				aPIndentArray.clear();
				copy( aLinePIndentArray.begin(), aLinePIndentArray.end(), back_inserter(aPIndentArray) );
				...
				aLineFontArray.clear();
				copy( aFontArray.begin(), aFontArray.end(), back_inserter(aLineFontArray) );
				...
				aLineColorArray.clear();
				copy( aColorArray.begin(), aColorArray.end(), back_inserter(aLineColorArray) );
				...
				aLinePIndentArray.clear();
				copy( aPIndentArray.begin(), aPIndentArray.end(), back_inserter(aLinePIndentArray) );
...
TImageInfo* CRenderEngine::LoadImageFromBuffer( LPBYTE pData, DWORD dwSize, DWORD mask )
{
	LPBYTE pImage = NULL;
	int x,y,n;
