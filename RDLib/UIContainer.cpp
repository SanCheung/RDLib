...
{
    if( EQUSTR(pstrName, _T("IContainer")) ) 
		return static_cast<IContainerUI*>(this);

    if( EQUSTR(pstrName, _T("Container")) )
...
void CContainerUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    if( EQUSTR(pstrName, _T("inset")) )
	{
		...
		RECT	rcInset = FormatToRect( pstrValue );
		SetInset(rcInset);
    }
    else if( EQUSTR(pstrName, _T("mousechild")) ) 
		SetMouseChildEnabled( EQUSTR_TRUE(pstrValue) );
    else if( EQUSTR(pstrName, _T("vscrollbar")) )
        EnableScrollBar( EQUSTR_TRUE(pstrValue), GetHorizontalScrollBar() != NULL);
    else if( EQUSTR(pstrName, _T("vscrollbarstyle")) )
	{
        EnableScrollBar(true, GetHorizontalScrollBar() != NULL);
        if( GetVerticalScrollBar() ) 
			GetVerticalScrollBar()->ApplyAttributeList(pstrValue);
    }
    else if( EQUSTR(pstrName, _T("hscrollbar")) )
	{
        EnableScrollBar(GetVerticalScrollBar() != NULL, EQUSTR_TRUE(pstrValue) );
    }
    else if( EQUSTR(pstrName, _T("hscrollbarstyle")) )
	{
    ...
    else if( EQUSTR(pstrName, _T("childpadding")) ) 
...
void CContainerUI::ProcessHScrollbar( RECT rc, int cxRequired )
{
	...
	// Scroll not needed anymore?
	int cyScroll = cxRequired - (rc.right - rc.left);
	if( cyScroll <= 0 && !m_bScrollProcess) {
	...
	else
	{
		...
		if( m_pHorizontalScrollBar->GetScrollRange() != cyScroll ) {
			int iScrollPos = m_pHorizontalScrollBar->GetScrollPos();
			m_pHorizontalScrollBar->SetScrollRange(::abs(cyScroll));
...
{
    if( EQUSTR(pstrName, _T("VerticalLayout")) ) 
...
void CVerticalLayoutUI::SetPos(RECT rc)
{
	...
	for( int it1 = 0; it1 < m_items.GetSize(); it1++ ) 
	{
		...
		if( sz.cy == 0 ) {
			nAdjustables++; //为0的控件平分剩下的高度
		}
		else {
			if( sz.cy < pControl->GetMinHeight() ) sz.cy = pControl->GetMinHeight(); //控件自身提供的最小和最大高度
			if( sz.cy > pControl->GetMaxHeight() ) sz.cy = pControl->GetMaxHeight();
		}
		cyFixed += sz.cy + pControl->GetPadding().top + pControl->GetPadding().bottom; //包括上下padding
		nEstimateNum++;
	}
	cyFixed += (nEstimateNum - 1) * m_iChildPadding; //计算padding

	// Place elements
	int cyNeeded = 0; //用于调整垂直滚动条,这个值是所有控件的高度+padding的和
	int cyExpand = 0; //平分剩余高度的控件的平分高
	if( nAdjustables > 0 ) 
	...
	int iPosY = rc.top;
	if( m_pVerticalScrollBar && m_pVerticalScrollBar->IsVisible() ) {
		iPosY -= m_pVerticalScrollBar->GetScrollPos(); //滚动条偏移
	}
	...
	for( int it2 = 0; it2 < m_items.GetSize(); it2++ ) {
		...
		if( sz.cy == 0 ) {
			...
			// Distribute remaining to last element (usually round-off left-overs) 它的意思是最后一个控件占用后面剩下的高度
			if( iAdjustable == nAdjustables ) {
				sz.cy = MAX(0, szRemaining.cy - rcPadding.bottom - cyFixedRemaining); //ok
			} 
	...
	ProcessScrollBar(rc, szAvailable.cx, cyNeeded);
}
...
{
    if( EQUSTR(pstrName, _T("sepheight")) ) 
		SetSepHeight(_ttoi(pstrValue) );
    else if( EQUSTR(pstrName, _T("sepimm")) )
		SetSepImmMode( EQUSTR_TRUE(pstrValue) );
    else 
...
{
    if( EQUSTR(pstrName, _T("HorizontalLayout")) ) 
...
void CHorizontalLayoutUI::SetPos(RECT rc)
{
	...
	for( int it2 = 0; it2 < m_items.GetSize(); it2++ ) {
		...
		cxNeeded += sz.cx + m_iChildPadding + rcPadding.left + rcPadding.right;
		cyNeeded = max( cyNeeded, sz.cy );
...
{
    if( EQUSTR(pstrName, _T("sepwidth")) ) 
...
{
    if( EQUSTR(pstrName, _T("TileLayout")) ) 
...
{
    if( EQUSTR(pstrName, _T("itemsize")) ) 
	{
		SIZE szItem = { 0 };
		LPTSTR pstr = NULL;
    ...
    else if( EQUSTR(pstrName, _T("columns")) ) 
...
{
    if( EQUSTR(pstrName, _T("TabLayout")) )
...
{
    if( EQUSTR(pstrName, _T("selectedid")) )
