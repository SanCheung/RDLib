	...
	, m_iExpandedItem(-1)
{
    ...
    m_ListInfo.dwBkColor = 0xFFf3f3f3;
    m_ListInfo.bAlternateBk = true;
...
{
	if( EQUSTR(pstrName, _T("List")) ) 
		return static_cast<CListUI*>(this);

    if( EQUSTR(pstrName, _T("IList")) ) 
		return static_cast<IListUI*>(this);

    if( EQUSTR(pstrName, _T("IListOwner")) ) 
...
void CListUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    ...
    if( EQUSTR(pstrName, _T("header")) )				pHeader->SetVisible( !EQUSTR(pstrValue, _T("hidden")) );
    else if( EQUSTR(pstrName, _T("headerbkimage")) )	pHeader->SetBkImage(pstrValue);
    else if( EQUSTR(pstrName, _T("scrollselect")) )		SetScrollSelect(EQUSTR_TRUE(pstrValue));
    else if( EQUSTR(pstrName, _T("multiexpanding")) )	SetMultiExpanding(EQUSTR_TRUE(pstrValue));
    else if( EQUSTR(pstrName, _T("itemfont")) )			m_ListInfo.nFont = _ttoi(pstrValue);
    else if( EQUSTR(pstrName, _T("itemalign")) ) 
	{
    ...
    else if( EQUSTR(pstrName, _T("itemendellipsis")) )
	{
        if( EQUSTR_TRUE(pstrValue) )
			m_ListInfo.uTextStyle |= DT_END_ELLIPSIS;
        else 
			m_ListInfo.uTextStyle &= ~DT_END_ELLIPSIS;
    }    
    if( EQUSTR(pstrName, _T("itemtextpadding")) ) 
	{ 
		RECT rcTextPadding = FormatToRect( pstrValue );
        SetItemTextPadding(rcTextPadding);
    }
    else if( EQUSTR(pstrName, _T("itemtextcolor")) ) 
	{
		DWORD	cr = FormatToColor( pstrValue );
		SetItemTextColor( cr );
    }
    else if( EQUSTR(pstrName, _T("itembkcolor")) ) 
	{
        DWORD	cr = FormatToColor( pstrValue );
		SetItemBkColor( cr );
    }
    else if( EQUSTR(pstrName, _T("itembkimage")) )		SetItemBkImage(pstrValue);
    else if( EQUSTR(pstrName, _T("itemaltbk")) )		SetAlternateBk(EQUSTR_TRUE(pstrValue));
    else if( EQUSTR(pstrName, _T("itemselectedtextcolor")) ) 
	{
        DWORD	cr = FormatToColor( pstrValue );
        SetSelectedItemTextColor( cr );
    }
    else if( EQUSTR(pstrName, _T("itemselectedbkcolor")) )
	{
		DWORD	cr = FormatToColor( pstrValue );
        SetSelectedItemBkColor( cr );
    }
    else if( EQUSTR(pstrName, _T("itemselectedimage")) )	SetSelectedItemImage(pstrValue);
    else if( EQUSTR(pstrName, _T("itemhottextcolor")) )
	{
		DWORD	cr = FormatToColor( pstrValue );
        SetHotItemTextColor( cr );
    }
    else if( EQUSTR(pstrName, _T("itemhotbkcolor")) )
	{
        DWORD	cr = FormatToColor( pstrValue );
        SetHotItemBkColor( cr );
    }
    else if( EQUSTR(pstrName, _T("itemhotimage")) )			SetHotItemImage(pstrValue);
    else if( EQUSTR(pstrName, _T("itemdisabledtextcolor")) )
	{
        DWORD	cr = FormatToColor( pstrValue );
        SetDisabledItemTextColor( cr );
    }
    else if( EQUSTR(pstrName, _T("itemdisabledbkcolor")) )
	{
        DWORD	cr = FormatToColor( pstrValue );
        SetDisabledItemBkColor( cr );
    }
    else if( EQUSTR(pstrName, _T("itemdisabledimage")) )	SetDisabledItemImage(pstrValue);
    else if( EQUSTR(pstrName, _T("itemlinecolor")) )
	{
        DWORD	cr = FormatToColor( pstrValue );
        SetItemLineColor( cr );
    }
    else if( EQUSTR(pstrName, _T("itemshowhtml")) )			SetItemShowHtml( EQUSTR_TRUE(pstrValue) );
    else 
...
{
    if( EQUSTR(pstrName, _T("ListHeader")) )
		return this;
...
void CListHeaderUI::DoPaint(HDC hDC, const RECT& rcPaint)
{
	//CDUIRect	rt = rcPaint;
	//rt.bottom = rt.top + 20;
	//CRenderEngine::DrawColor( hDC, rt, 0xffccccFF );
...
{
    if( EQUSTR(pstrName, _T("ListHeaderItem")) )
		return this;
...
{
    if( EQUSTR(pstrName, _T("dragable")) )			SetDragable(EQUSTR(pstrValue, _T("true")));
    else if( EQUSTR(pstrName, _T("sepwidth")) )		SetSepWidth(_ttoi(pstrValue));
    else if( EQUSTR(pstrName, _T("align")) )
	{
        if( _tcsstr(pstrValue, _T("left")) != NULL )
		{
	...
	{
        if( EQUSTR(pstrValue, _T("true")) ) m_uTextStyle |= DT_END_ELLIPSIS;
        else m_uTextStyle &= ~DT_END_ELLIPSIS;
    }    
    else if( EQUSTR(pstrName, _T("font")) ) SetFont(_ttoi(pstrValue));
	...
	{
		RECT	rc = FormatToRect( pstrValue );
		SetTextPadding( rc );
	}
    else if( EQUSTR(pstrName, _T("showhtml")) )			SetShowHtml( EQUSTR_TRUE(pstrValue) );
    else if( EQUSTR(pstrName, _T("normalimage")) )		SetNormalImage(pstrValue);
    else if( EQUSTR(pstrName, _T("hotimage")) )			SetHotImage(pstrValue);
    else if( EQUSTR(pstrName, _T("pushedimage")) )		SetPushedImage(pstrValue);
    else if( EQUSTR(pstrName, _T("focusedimage")) )		SetFocusedImage(pstrValue);
    else if( EQUSTR(pstrName, _T("sepimage")) )			SetSepImage(pstrValue);
...
void CListHeaderItemUI::DoEvent(TEventUI& event)
{
    ...
    if( event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_DBLCLICK )
    {
		...
		if (m_iSepWidth>=0)//111024 by cddjr, ???¡§??¡è????¡è???¡§?¡§¡ã?¨º??¡è??¨¤?¡§????¨¬¡§a???£¤
			rcSeparator.left-=4;
		else
			rcSeparator.right+=4;
...
void CListHeaderItemUI::DoPaint(HDC hDC, const RECT& rcPaint)
{
	...
	CRenderEngine::DrawText( hDC, m_pManager, rcMessage, m_sText, m_dwTextColor, m_iFont, DT_SINGLELINE | m_uTextStyle);
	
	
	// Draw gripper
	POINT ptTemp = { 0 };
	//RECT rcThumb = GetThumbRect(m_rcItem);
	CDUIRect rcThumb(m_rcItem.right - 4, m_rcItem.top, m_rcItem.right, m_rcItem.bottom - 3);

	RECT rc1 = { rcThumb.left + 2, rcThumb.top + 4, rcThumb.left + 2, rcThumb.bottom - 1 };
	CRenderEngine::DrawLine( hDC, rc1, 0, 0xff7F7F7F );//UICOLOR_HEADER_SEPARATOR );
	...
	CRenderEngine::DrawLine( hDC, rc1, 0, 0xffFFFFFF );//UICOLOR_STANDARD_WHITE );
...
LPVOID CListElementUI::GetInterface(LPCTSTR pstrName)
{
	...
	if( EQUSTR(pstrName, _T("ListElement")) ) 
...
CListLabelElementUI::CListLabelElementUI( LPCWSTR text, LPCWSTR userData )
{
	SetText(text);
...
{
    if( EQUSTR(pstrName, _T("ListLabelElement")) ) 
...
SIZE CListLabelElementUI::EstimateSize(SIZE szAvailable)
{
    ...
    if( cXY.cx == 0 && m_pManager != NULL ) {
        ...
        if( pInfo->bShowHtml ) {
            int nLinks = 0;
            CRenderEngine::DrawHtmlText(m_pManager->GetPaintDC(), m_pManager, rcText, m_sText, 0, \
				pInfo->nFont, NULL, NULL, nLinks, DT_SINGLELINE | DT_CALCRECT | pInfo->uTextStyle & ~DT_RIGHT & ~DT_CENTER);
        }
        else {
...
void CListLabelElementUI::DrawItemText(HDC hDC, const RECT& rcItem)
{
    ...
    if( pInfo->bShowHtml )
        CRenderEngine::DrawHtmlText(hDC, m_pManager, rcText, m_sText, iTextColor, \
			pInfo->nFont, NULL, NULL, nLinks, DT_SINGLELINE | pInfo->uTextStyle);
    else
...
{
    if( EQUSTR(pstrName, _T("ListTextElement")) ) 
...
CStringW* CListTextElementUI::GetLinkContent(int iIndex)
{
    ...
    for( int i = 0; i < pInfo->nColumns; i++ )
    {
        ...
        if( pInfo->bShowHtml )
            CRenderEngine::DrawHtmlText(hDC, m_pManager, rcItem, pstrText, iTextColor, \
                pInfo->nFont, &m_rcLinks[m_nLinks], &m_sLinks[m_nLinks], nLinks, DT_SINGLELINE | pInfo->uTextStyle);
        else
...
LPVOID CListContainerElementUI::GetInterface(LPCTSTR pstrName)
{
	...
	if( EQUSTR(pstrName, _T("ListContainerElement")) ) 
